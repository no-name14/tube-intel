  ...
  async fetch(request) {
    ...
    if (url.pathname === '/ai') {
      ...
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        method: 'POST',
        headers: {'Content-Type':'application/json','x-api-key':k1+k2+k3,'anthropic-version':'2023-06-01'},
...
<!-- SETUP（非表示・認証情報埋め込み済み）-->
<div id="setup" style="display:none">
  <div class="err-box" id="err-box"></div>
  <input type="hidden" id="f-apiKey">
  <input type="hidden" id="f-clientId">
  <input type="hidden" id="f-clientSecret">
  <input type="hidden" id="f-refreshToken">
  <input type="hidden" id="f-channel">
</div>
...
function showErr(m){const e=document.getElementById('err-box');if(e){e.textContent='⚠ '+m;e.style.display='block';}else{alert('⚠ '+m);}}
function hideErr(){const e=document.getElementById('err-box');if(e)e.style.display='none';}
